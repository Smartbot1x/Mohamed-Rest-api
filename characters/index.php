...
// (with optional type filter for separation: ?type=demon or ?type=slayer)
if ($_SERVER["REQUEST_METHOD"] === "GET" && empty($_GET["id"])) {
	...
	for ($i = 0; $i < count($results); $i++) {
		$results[$i]["url"] = "http://localhost/demon-slayer/characters?id=" . $results[$i]["id"];
		unset($results[$i]["id"]);
	}
...
if ($_SERVER["REQUEST_METHOD"] === "GET" && !empty($_GET["id"])) {
	...
	// TEMP: Simplified query without JOINs (until media tables are created)
	// Uncomment the full JOIN below once tables exist
	$stmt = $conn->prepare("SELECT id, name, description, rank, type, abilities FROM characters WHERE id = :id");
	/*
	...
	*/
	...
	/*
	// TEMP: Comment out the media loop until JOIN is back
	$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
